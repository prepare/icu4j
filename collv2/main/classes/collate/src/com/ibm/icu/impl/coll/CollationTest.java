...
boolean CollationTest.checkCompareTwo(const char *norm, const UnicodeString &prevFileLine,
    ...
    if(&prevValid == &prevString && &sValid == &s) {
        ...
        expectedUTF8Order = coll.compare(prevValid, sValid);
    }
    ...
    if(order != expectedUTF8Order || errorCode.isFailure()) {
        ...
        errln("line %d Collator(%s).compareUTF8(previous, current) wrong order: %d != %d (%s)",
              (int)fileLineNumber, norm, order, expectedUTF8Order.errorName());
        infoln(prevFileLine);
        infoln(fileLine);
        infoln(printCollationKey(prevKey));
        infoln(printCollationKey(key));
    ...
    order = coll.compareUTF8(sUTF8, prevUTF8);
    if(order != -expectedUTF8Order || errorCode.isFailure()) {
        infoln(fileTestName);
    ...
    // Test NUL-termination if the strings do not contain NUL characters.
    if(!containNUL) {
        order = coll.internalCompareUTF8(prevUTF8.c_str(), -1, sUTF8.c_str(), -1);
        if(order != expectedUTF8Order || errorCode.isFailure()) {
            infoln(fileTestName);
        ...
        order = coll.internalCompareUTF8(sUTF8.c_str(), -1, prevUTF8.c_str(), -1);
        if(order != -expectedUTF8Order || errorCode.isFailure()) {
            infoln(fileTestName);
