 ...
 */
final class CollationWeights {
public:
    CollationWeights();

    static int lengthOfWeight(uint32_t weight) {
    ...
    void initForPrimary(boolean compressible);
     ...
     */
    boolean allocWeights(uint32_t lowerLimit, uint32_t upperLimit, int n);
...
private:
    ...
    int countBytes(int idx) {
        return (int32_t)(maxBytes[idx] - minBytes[idx] + 1);
    }

    uint32_t incWeight(uint32_t weight, int length);
    uint32_t incWeightByOffset(uint32_t weight, int length, int offset);
    void lengthenRange(WeightRange &range);
    /**
     * Takes two CE weights and calculates the
     * possible ranges of weights between the two limits, excluding them.
     * For weights with up to 4 bytes there are up to 2*4-1=7 ranges.
     */
    boolean getWeightRanges(uint32_t lowerLimit, uint32_t upperLimit);
    boolean allocWeightsInShortRanges(int n, int minLength);
    boolean allocWeightsInMinLengthRanges(int n, int minLength);
...
*   indentation:4
*
*   @since 2001mar08 as ucol_wgt.cpp
*   @author Markus W. Scherer
*
...
static uint32_t
getWeightTrail(uint32_t weight, int length) {
    return (uint32_t)(weight>>(8*(4-length)))&0xff;
}

static uint32_t
setWeightTrail(uint32_t weight, int length, uint32_t trail) {
    length=8*(4-length);
    return (uint32_t)((weight&(0xffffff00<<length))|(trail<<length));
}

static uint32_t
getWeightByte(uint32_t weight, int idx) {
    return getWeightTrail(weight, idx); /* same calculation */
}

static uint32_t
setWeightByte(uint32_t weight, int idx, uint32_t byte) {
...
static uint32_t
truncateWeight(uint32_t weight, int length) {
    return (uint32_t)(weight&(0xffffffff<<(8*(4-length))));
}

static uint32_t
incWeightTrail(uint32_t weight, int length) {
    return (uint32_t)(weight+(1UL<<(8*(4-length))));
}

static uint32_t
decWeightTrail(uint32_t weight, int length) {
    return (uint32_t)(weight-(1UL<<(8*(4-length))));
}
...
void
CollationWeights.initForPrimary(boolean compressible) {
    middleLength=1;
...
uint32_t
CollationWeights.incWeight(uint32_t weight, int length) {
    for(;;) {
...
uint32_t
CollationWeights.incWeightByOffset(uint32_t weight, int length, int offset) {
    for(;;) {
...
boolean
CollationWeights.allocWeightsInShortRanges(int n, int minLength) {
...
boolean
CollationWeights.allocWeightsInMinLengthRanges(int n, int minLength) {
 ...
 */
boolean
CollationWeights.allocWeights(uint32_t lowerLimit, uint32_t upperLimit, int n) {
#ifdef UCOL_DEBUG
    puts("");
#endif
