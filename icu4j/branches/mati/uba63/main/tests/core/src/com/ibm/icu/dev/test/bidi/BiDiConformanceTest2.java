 ...
 * adapted from BiDiConformanceTest.java originally written by Markus W. Scherer
 * BiDi conformance test, using the Unicode BidiCharacterTest.txt file.
 * Ported from ICU4C intltest/bidiconf2.cpp .
 */
...
public class BiDiConformanceTest2 extends TestFmwk {
    public static void main(String[] args) throws Exception {
        new BiDiConformanceTest2().run(args);
    }
    public BiDiConformanceTest2() {}

    public void TestBidiTest2() throws IOException {
...
outerLoop:
        while(errorCount<20 && (line=bidiTestFile.readLine())!=null) {
            ...
            if(false==parseInputStringFromBiDiClasses(parts[0].trim())) {
                continue;
            }
            ...
            if(false==parseLevels(parts[3].trim())) {
                continue;
            }
            if(parts.length>4) {
                if(false==parseOrdering(parts[4].trim()))
                    continue;
            }
            else
                orderingCount=-1;
    ...
    private boolean parseLevels(String line) {
        directionBits=0;
        levelsCount=0;
        ...
        for(String levelString: levelStrings) {
            if(levelString.equals("x")) {
                ...
                int value=intFromString(levelString);
    ...
    private boolean parseOrdering(String line) {
        orderingCount=0;
        String[] orderingStrings=line.split("[ \t]+");
        for(String orderingString: orderingStrings) {
            int value=intFromString(orderingString);
    ...
    private boolean parseInputStringFromBiDiClasses(String line) {
        int i, j, lineIndex=0;
        ...
        while(lineIndex<line.length()) {
            c0=line.charAt(lineIndex);
            if(isInvWhitespace(c0)) {
                lineIndex++;
                continue;
            }
            // First look for an hexa value of at least 2 digits
            for(i=lineIndex; i<line.length() && !isInvWhitespace(line.charAt(i)); i++);

            if((i-lineIndex)>=2) {      /* token is at least 2 chars */
                int d1=-1, hexnum=0;

                for(j=lineIndex; j<i; j++) {
                    d1=hexdigit(line.charAt(j));
                    if(d1<0)
                        break;
                    hexnum=(hexnum<<4) + d1;
                }
                if(d1>=0) {             /* valid hex number */
                    if(hexnum>0xffff) {
                        errln(" ------------ Hexa number too large at '"
                                         +line.substring(lineIndex,i)+"'");
                        printErrorLine();
                    }
                    inputStringBuilder.append((char)hexnum);
                    lineIndex=i;
                    continue;
                }
            }
            ...
            if( biDiClass<UCharacterDirection.CHAR_DIRECTION_COUNT &&
                ((lineIndex+biDiClassNameLength)>=line.length() ||
                 isInvWhitespace(line.charAt(lineIndex+biDiClassNameLength)))
            ) {
            ...
            if((lineIndex+1)==line.length() ||
               isInvWhitespace(line.charAt(lineIndex+1))) {
                inputStringBuilder.append(line.charAt(lineIndex));
                lineIndex++;
                continue;
            }
            errln(" ------------ BiDi class string not recognized at '"
