...
{   
	...
	/** 
	 * Method to retrieve the scripts equivalent to the given script for reordering.
	 * Some scripts will share the same "lead byte" used for the collation codes and so must
	 * be reordered together.
	 * @see #setScriptOrder
	 * @see #setScriptOrderDefault
	 * @param reorderCode
...
//			return false;
//		if (m_bottom3_ != other.m_bottom3_)
//			return false;
//		if (m_bottomCount3_ != other.m_bottomCount3_)
//			return false;
//		if (m_common3_ != other.m_common3_)
//			return false;
//		if (m_isSimple3_ != other.m_isSimple3_)
//			return false;
//		if (m_mask3_ != other.m_mask3_)
//			return false;
//		if (m_minContractionEnd_ != other.m_minContractionEnd_)
//			return false;
//		if (m_minUnsafe_ != other.m_minUnsafe_)
//			return false;
//		if (m_top3_ != other.m_top3_)
//			return false;
//		if (m_topCount3_ != other.m_topCount3_)
//			return false;
		for (int i = 0; i < this.m_unsafe_.length; i++) {
			if (this.m_unsafe_[i] != other.m_unsafe_[i]) {
				System.out.println(i + " = " + this.m_unsafe_[i] + ", " + other.m_unsafe_[i]);
			}
		}
		if (!Arrays.equals(m_unsafe_, other.m_unsafe_)) {
			return false;
		}
		if (m_addition3_ != other.m_addition3_
	...
	static final class LeadByteConstants
	{
		...
		void read(DataInputStream dis) throws IOException {
			...
			for (int index = 0; index < indexCount; index++) {
				this.LEAD_BYTE_TO_SCRIPTS_INDEX[index] = dis.readShort();
				readcount += 2;
			}
		...
		private static int readShort(byte[] data, int offset) {
			return data[offset * 2] << 8 + (0xff & data[(offset + 1) * 2]);
		}
	}
...
//	final static int[][] m_leadByteToScripts_old;
//	final static int[][] m_scriptToLeadBytes_old;
//	// TODO(sgill) - the work of converting script names to codes should be done in the genuca tool
//	static {
//		// script code to list of lead bytes
//		SortedMap<Integer, List<Integer>> scriptToLeadBytesMap = new TreeMap<Integer, List<Integer>>();
//		for (int script = 0; script < SCRIPTS_FOR_LEAD_BYTES.length; script++) {
//			List<Integer> tempLeadBytes = new ArrayList<Integer>();
//			for (int leadByte : LEAD_BYTES_BY_SCRIPT[script]) {
//				tempLeadBytes.add(leadByte);
//			}
//			if (!SCRIPTS_FOR_LEAD_BYTES[script].equals("Undefined")) {
//				scriptToLeadBytesMap.put(UScript.getCode(SCRIPTS_FOR_LEAD_BYTES[script])[0], tempLeadBytes);
//			}
//		}
//		final int[][] temp_scriptToLeadBytes = new int[UScript.CODE_LIMIT][];
//		for (Integer script : scriptToLeadBytesMap.keySet()) {
//			List<Integer> leadBytes = scriptToLeadBytesMap.get(script);
//			temp_scriptToLeadBytes[script] = new int[leadBytes.size()];
//			for (int leadByte = 0; leadByte < leadBytes.size(); leadByte++) {
//				temp_scriptToLeadBytes[script][leadByte] = leadBytes.get(leadByte);
//			}
//		}
//		m_scriptToLeadBytes_old = temp_scriptToLeadBytes;
//
//		// lead byte to list of script codes
//		int[][] temp_leadByteToScripts = new int[LEAD_BYTE_TO_SCRIPTS.length][];
//		for (int leadByte = 0; leadByte < LEAD_BYTE_TO_SCRIPTS.length; leadByte++) {
//			temp_leadByteToScripts[leadByte] = new int[LEAD_BYTE_TO_SCRIPTS[leadByte].length];
//			for (int script = 0; script < LEAD_BYTE_TO_SCRIPTS[leadByte].length; script++) {
//				// TODO(sgill) - could take all of them but will need to use a list for conversion and then copy it out to the array
//				temp_leadByteToScripts[leadByte][script] = LEAD_BYTE_TO_SCRIPTS[leadByte][script].equals("Undefined") ?
//						UScript.INVALID_CODE : UScript.getCode(LEAD_BYTE_TO_SCRIPTS[leadByte][script])[0];
//			}
//			Arrays.sort(temp_leadByteToScripts[leadByte]);
//		}
//		m_leadByteToScripts_old = temp_leadByteToScripts;
//	}
	...
	private void buildScriptReorderTable() {
		...
		if (m_scriptOrder_ != null) {
			...
			//                if (m_scriptOrder_[next] != UScript.UNKNOWN) {
			//                    for (int i = 0; i < 256; i++){
			//                        while (m_leadByteToScripts[toBottom][0] == UScript.INVALID_CODE) {
			//                            filled[toBottom] = true;
			//                            m_leadBytePermutationTable_[toBottom] = (byte)toBottom++;
			//                        }
			//                        if (Arrays.binarySearch(m_leadByteToScripts[i], m_scriptOrder_[next]) >= 0) {
			//                            filled[i] = true;
			//                            m_leadBytePermutationTable_[i] = (byte)toBottom++;
			//                        }
			//                    }
			//                } else {
			//                    int last = m_scriptOrder_.length-1;
			//                    while (last > next) {
			//                        for (int i = 255; i >= 0; i--) {
			//                            while (m_leadByteToScripts[toTop][0] == UScript.INVALID_CODE) {
			//                                filled[toTop] = true;
			//                                m_leadBytePermutationTable_[toTop] = (byte)toTop--;
			//                            }
			//                            if (Arrays.binarySearch(m_leadByteToScripts[i], m_scriptOrder_[last]) >= 0) {
			//                                filled[i] = true;
			//                                m_leadBytePermutationTable_[i] = (byte)toTop--;
			//                            }
			//                        }
			//                        --last;
			//                    }
			...
			while (next < m_scriptOrder_.length) {
				...
				int[] leadBytes = RuleBasedCollator.LEADBYTE_CONSTANTS_.getLeadBytesForReorderCode(m_scriptOrder_[next]);
				if (fromTheBottom) {
					for (int leadByte : leadBytes) {
						...
						// skip unmovable lead bytes
						int[] reorderCodes;
						for (reorderCodes = RuleBasedCollator.LEADBYTE_CONSTANTS_.getReorderCodesForLeadByte(toBottom);
							reorderCodes[0] == UScript.INVALID_CODE;
							toBottom++, reorderCodes = RuleBasedCollator.LEADBYTE_CONSTANTS_.getReorderCodesForLeadByte(toBottom))
						{
							m_leadBytePermutationTable_[toBottom] = (byte) toBottom;
							filled[toBottom] = true;
						}

						m_leadBytePermutationTable_[toBottom] = (byte) leadByte;
						used[leadByte] = true;
						filled[toBottom] = true;
						toBottom++;
					}
					...
					for (int leadByteIndex = leadBytes.length - 1; leadByteIndex >= 0; leadByteIndex--) {
						...
						// skip unmovable lead bytes
						int[] reorderCodes;
						for (reorderCodes = RuleBasedCollator.LEADBYTE_CONSTANTS_.getReorderCodesForLeadByte(toTop);
							reorderCodes[0] == UScript.INVALID_CODE;
							toTop--, reorderCodes = RuleBasedCollator.LEADBYTE_CONSTANTS_.getReorderCodesForLeadByte(toTop))
						{
							m_leadBytePermutationTable_[toTop] = (byte) toTop;
							filled[toTop] = true;
						}
						m_leadBytePermutationTable_[toTop] = (byte) leadBytes[leadByteIndex];
