...
public class UnicodeSet extends UnicodeFilter implements Iterable<String>, Comparable<UnicodeSet>, Freezable<UnicodeSet> {
    ...
    public boolean containsNone(String s) {
        return (span(s, s.length(), USetSpanCondition.USET_SPAN_NOT_CONTAINED) == s.length());
    }
     ...
     * @stable ICU 3.8
     */
    public int span(final String s, int limit, USetSpanCondition spanCondition) {
        return span(s, 0, limit, spanCondition);
    }
     ...
     * @stable ICU 3.8
     */
    public int span(final String s, int start, int limit, USetSpanCondition spanCondition) {
        ...
        if (stringSpan != null) {
            ...
            int which = spanCondition == USetSpanCondition.USET_SPAN_NOT_CONTAINED ? UnicodeSetStringSpan.FWD_UTF16_NOT_CONTAINED
        ...
        // Pin to 0/1 values.
        boolean spanContained = (spanCondition != USetSpanCondition.USET_SPAN_NOT_CONTAINED);
    ...
    /**
     * Span a string backwards (from the last char) using this UnicodeSet.
     * 
     * @param s The string to be spanned
     * @param limit The limit of the number of chars that the string should be spanned
     * @param spanCondition The span condition
     ...
     * @stable ICU 3.8
     */
    public int spanBack(final String s, int limit, USetSpanCondition spanCondition) {
        if (limit > 0 && bmpSet != null) {
        ...
        if (stringSpan != null) {
            ...
            int which = (spanCondition == USetSpanCondition.USET_SPAN_NOT_CONTAINED)
     ...
     * case): - USET_SPAN_CONTAINED and USET_SPAN_SIMPLE work the same. - span() and spanBack() partition any string the
     * same way when alternating between span(USET_SPAN_NOT_CONTAINED) and span(either "contained" condition). - Using a
     ...
     * because a set string may start before the later position. - span(USET_SPAN_SIMPLE) may be shorter than
     * span(USET_SPAN_CONTAINED) because it will not recursively try all possible paths. For example, with a set which
     * contains the three strings "xy", "xya" and "ax", span("xyax", USET_SPAN_CONTAINED) will return 4 but span("xyax",
     * USET_SPAN_SIMPLE) will return 3. span(USET_SPAN_SIMPLE) will never be longer than span(USET_SPAN_CONTAINED). -
     ...
     * parent set.
     */
    static final class BMPSet {
        public static int U16_SURROGATE_OFFSET = ((0xd800 << 10) + 0xdc00 - 0x10000);

        /*
         * One byte per ASCII character, or trail byte in lead position. 0 or 1 for ASCII characters [0 .. 0x7F]. The
        ...
        public BMPSet(final BMPSet otherBMPSet, final int[] newParentList, int newParentListLength) {
            ...
            asciiBytes = otherBMPSet.asciiBytes.clone();
        ...
        /*
         * Span the initial substring for which each character c has spanCondition==contains(c). It must be s.length >=
         ...
         * as usual in ICU.
         */
        public final int span(final String s, int limit, USetSpanCondition spanCondition) {
            return span(s, 0, limit, spanCondition);
        }

        public final int span(final String s, int start, int limit, USetSpanCondition spanCondition) {
            char c, c2;
            int i = start;
            limit = Math.min(s.length(), start + limit);
            if (USetSpanCondition.USET_SPAN_NOT_CONTAINED != spanCondition) {
                // span
                while (i < limit) {
                    c = s.charAt(i);
                    if (c <= 0x7f) {
         ...
         * @return The string index which starts the span (i.e. inclusive).
         */
        public final int spanBack(final String s, int limit, USetSpanCondition spanCondition) {
            char c, c2;
         ...
         * to set 0 values where they were reset to 0 in the constructor and not modified by initBits(). (asciiBytes[]
         * trail bytes, table7FF[] 0..7F, bmpBlockBits[] 0..7FF) Need to set 0 values for surrogates D800..DFFF.
         */
        ...
        private int findCodePoint(int c, int lo, int hi) {
            /*
             * Examples: findCodePoint(c) set list[] c=0 1 3 4 7 8 === ============== =========== [] [110000] 0 0 0 0 0
             * 0 [\u0000-\u0003] [0, 4, 110000] 1 1 1 2 2 2 [\u0004-\u0007] [4, 8, 110000] 0 0 0 1 1 2 [:Any:] [0,
             * 110000] 1 1 1 1 1 1
             */
