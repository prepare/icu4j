 ...
 */
public final class ICUResReader implements ICUBinary.Authenticate {
    /**
    ...
    private ICUResReader(InputStream stream, String resolvedName){
    ...
    static ICUResReader getReader(String resolvedName, ClassLoader root) {
        InputStream stream = ICUData.getStream(root,resolvedName);
        
        if(stream==null){
            return null;
        }
        ICUResReader reader = new ICUResReader(stream, resolvedName);
        return reader;
    }
    
    void setPoolBundleKeys(ICUResReader poolBundleReader) {
    ...
    static boolean URES_IS_TABLE(int type) {
        return type==ICUResConstants.TABLE || type==ICUResConstants.TABLE16 || type==ICUResConstants.TABLE32;
    }
    ...
    String getString(int res) {
        ...
        int length;
        if(RES_GET_TYPE(res)==ICUResConstants.STRING_V2) {
    ...
    String getAlias(int res) {
        int offset=RES_GET_OFFSET(res);
        int length;
        if(RES_GET_TYPE(res)==ICUResConstants.ALIAS) {
            if(offset==0) {
                return emptyString;
            } else {
                offset=getResourceByteOffset(offset);
    ...
    byte[] getBinary(int res, byte[] ba) {
        int offset=RES_GET_OFFSET(res);
        int length;
        if(RES_GET_TYPE(res)==ICUResConstants.BINARY) {
            if(offset==0) {
                return emptyBytes;
            } else {
                offset=getResourceByteOffset(offset);
    ...
    int[] getIntVector(int res) {
        int offset=RES_GET_OFFSET(res);
        int length;
        if(RES_GET_TYPE(res)==ICUResConstants.INT_VECTOR) {
            if(offset==0) {
                return emptyInts;
            } else {
                offset=getResourceByteOffset(offset);
    ...
    static class Container {
        ...
        int getContainerResource(int index) {
            return ICUResConstants.RES_BOGUS;
        }
        protected int getContainer16Resource(int index) {
            if (index < 0 || size <= index) {
                return ICUResConstants.RES_BOGUS;
            }
            return (ICUResConstants.STRING_V2 << 28) |
        ...
        Container(ICUResReader reader) {
            this.reader = reader;
        }

        ICUResReader getReader() {
            return reader;
        }
    }
    private static final class Array extends Container {
        ...
        Array(ICUResReader reader, int offset) {
            super(reader);
    ...
    private static final class Array16 extends Container {
        ...
        Array16(ICUResReader reader, int offset) {
            super(reader);
    ...
    private static final class Table1632 extends Table {
        ...
        Table1632(ICUResReader reader, int offset) {
            super(reader);
            offset = reader.getResourceByteOffset(offset);
    ...
    private static final class Table16 extends Table {
        ...
        Table16(ICUResReader reader, int offset) {
            super(reader);
    ...
    private static final class Table32 extends Table {
        ...
        Table32(ICUResReader reader, int offset) {
            super(reader);
            offset = reader.getResourceByteOffset(offset);
