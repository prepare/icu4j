...
public class StringRecord {
    ...
    // the pool
    private static SoftReference<StringRecord>[] recordRefs = new SoftReference[POOLSIZE];
    ...
    private BidiStructuredProcessor.StructuredTypes[] handlers;
    ...
    /**
     * Records a string in the pool. The caller must specify the number of
     * segments in the record (at least 1), and the handler, starting and ending
     * offsets for the first segment.
     * 
     * @param string
     *            the string to record.
     * @param segmentCount
     *            number of segments allowed in this string. This number must be
     *            >= 1.
     * @param handlerID
     *            identifier for the handler appropriate to handle the type of
     *            structured text present in the first segment. It may be one of
     *            the predefined identifiers in {@link TypeHandlerFactory}, or
     *            it may be an identifier for a type handler created by a
     *            plug-in or by the application.
     * @param start
     *            offset in the string of the starting character of the first
     *            segment. It must be >= 0 and less than the length of the
     *            string.
     * @param limit
     *            offset of the character following the first segment. It must
     *            be greater than the <code>start</code> argument and not
     *            greater than the length of the string.
     * 
     * @return an instance of RecordString which represents this record. This
     *         instance may be used to specify additional segments with
     *         {@link #addSegment addSegment}.
     * 
     * @throws IllegalArgumentException
     *             if <code>string</code> is null or if
     *             <code>segmentCount</code> is less than 1.
     * @throws also
     *             the same exceptions as {@link #addSegment addSegment}.
     */
    public static StringRecord addRecord(String string, int segmentCount,
            BidiStructuredProcessor.StructuredTypes handlerID, int start, int limit) {
        if (string == null)
            throw new IllegalArgumentException(
                    "The string argument must not be null!");
        if (segmentCount < 1)
            throw new IllegalArgumentException(
                    "The segment count must be at least 1!");
        ...
        if (record == null) {
            ...
            recordRefs[last] = new SoftReference<StringRecord>(record);
        }
        ...
        if (segmentCount > record.totalSegmentCount) {
            record.handlers = new BidiStructuredProcessor.StructuredTypes[segmentCount];
     ...
     *            plug-in or by the application.
     * @param start
     *            offset in the string of the starting character of the segment.
     *            It must be >= 0 and less than the length of the string.
     * @param limit
     *            offset of the character following the segment. It must be
     *            greater than the <code>start</code> argument and not greater
     *            than the length of the string.
     * 
     * @throws IllegalArgumentException
     *             if <code>handlerID</code> is null, or if <code>start</code>
     *             or <code>limit</code> have invalid values.
     * @throws IllegalStateException
     *             if the current segment exceeds the number of segments
     *             specified by <code>segmentCount</code> in the call to
     *             {@link #addRecord addRecord} which created the StringRecord
     *             instance.
     */
    public void addSegment(BidiStructuredProcessor.StructuredTypes handlerID, int start, int limit) {
        if (handlerID == null)
            throw new IllegalArgumentException(
                    "The handlerID argument must not be null!");
        if (start < 0 || start >= string.length())
            throw new IllegalArgumentException(
                    "The start position must be at least 0 and less than the length of the string!");
        if (limit <= start || limit > string.length())
            throw new IllegalArgumentException(
                    "The limit position must be greater than the start position but no greater than the length of the string!");
        if (usedSegmentCount >= totalSegmentCount)
            throw new IllegalStateException(
                    "All segments of the record are already used!");
     ...
     *         otherwise, return the <code>StringRecord</code> instance which
     *         records this string.<br>
     *         Once a record has been found, the number of its segments can be
     *         retrieved using {@link #getSegmentCount getSegmentCount}, its
     *         handler ID can be retrieved using {@link #getHandler getHandler},
     *         its starting offset can be retrieved using {@link #getStart
     *         getStart}, its ending offset can be retrieved using
     *         {@link #getLimit getLimit},
     */
    public static StringRecord getRecord(String string) {
    ...
    private void checkSegmentNumber(int segmentNumber) {
        if (segmentNumber >= usedSegmentCount)
            throw new IllegalArgumentException("The segment number "
                    + segmentNumber
                    + " is greater than the total number of segments = "
                    + usedSegmentCount + "!");
    }
     ...
     * 
     * @param segmentNumber
     *            number of the segment about which information is required. It
     *            must be >= 0 and less than the number of segments returned by
     *            {@link #getSegmentCount}.
     * 
     * @return the handler ID of the handler appropriate to process the
     *         structured text in the segment specified by
     *         <code>segmentNumber</code>.
     * 
     * @throws IllegalArgumentException
     *             if <code>segmentNumber</code> has an invalid value.
     */
    public BidiStructuredProcessor.StructuredTypes getHandler(int segmentNumber) {
    ...
    public static synchronized void clear() {
        for (int i = 0; i <= MAXINDEX; i++) {
            hashArray[i] = 0;
            SoftReference<StringRecord> softRef = recordRefs[i];
            if (softRef == null)
                continue;
            StringRecord record = softRef.get();
            if (record == null)
                continue;
