...
public class CharTypes {
	...
	public byte getBidiTypeAt(int index) {
		...
		if (charType == B) {
			...
			charType = (direction == Environment.ORIENT_RTL) ? R : L;
		}
	...
	public int resolveOrientation() {
		...
		if ((orient & Environment.ORIENT_CONTEXTUAL) == 0) { // absolute
																// orientation
			return orient;
		}
		// contextual orientation:
		orient &= ~Environment.ORIENT_CONTEXTUAL; // initiate to the
													// default orientation
													// minus contextual bit
		int len = text.length();
		byte charType;
		for (int i = 0; i < len; i++) {
			...
			if (isLeftToRight(charType))
				return Environment.ORIENT_LTR;
			if (isRightToLeft(charType))
				return Environment.ORIENT_RTL;
		}
		return orient;
	}

	public byte charTypeForDirection(int direction) {
		if (direction < 0) {
			direction = getDirection();
		}
		return direction == Bidi.DIRECTION_RIGHT_TO_LEFT ? R : L;
	}
