...
public class ExpertImpl implements Expert {
	...
	static public void processSeparator(String text, CharTypes charTypes,
		...
		if (direction == Bidi.DIRECTION_RIGHT_TO_LEFT) {
	...
	private Offsets leanToFullCommon(String text, CharTypes charTypes) {
		...
		if (orientation == Environment.ORIENT_IGNORE)
			prefixLength = 0;
		else {
			int resolvedOrientation = charTypes.resolveOrientation();
			if (orientation != Environment.ORIENT_UNKNOWN
					&& resolvedOrientation == direction)
				prefixLength = 0;
			else if ((orientation & Environment.ORIENT_CONTEXTUAL) != 0)
				prefixLength = 1;
			else
				prefixLength = 2;
		}
	 ...
	 */
	public String insertMarks(String text, int[] offsets, int direction,
			int affixLength) {
