 ...
 *       the set of tests to perform was originally specified tothe SpoofChecker.
 *
 ...
 * They may called concurrently, from multiple threads, using the same SpoofChecker instance.
 *
 * More generally, the standard ICU thread safety rules apply:  functions that take a
 * SpoofChecker parameter are thread safe.  Those that take a non-
 ...
 *
 *   -# SINGLE_SCRIPT_CONFUSABLE:  All of the characters from the two idenifiers are
 ...
 *   -# WHOLE_SCRIPT_CONFUSABLE: Each of the two idenifiers is of a single script, but
 ...
 *       of acceptable characters.  See setAllowedChars() and
 *       setAllowedLocales().
 *
 *  Note on Scripts:
...
public class SpoofChecker {
  ...
  public static class SpoofCheckerException extends Exception {
    static final long serialVersionUID = 1660237078466535056L;
  }

  /**
   * Enum for the kinds of checks that USpoofChecker can perform. These enum
  ...
  /**
   * Mixed script confusable test. When checking a single identifier, report a
   * problem if the identifier contains multiple scripts, and is confusable
   * with some other identifer in a single script When testing whether two
   * identifiers are confusable, report that they are if the two IDs are
   * visually confusable, and and at least one contains characters from more
   * than one script.
   */
  public static final int MIXED_SCRIPT_CONFUSABLE = 2;

  /**
   * Whole script confusable test. When checking a single identifier, report a
   * problem if The identifier is of a single script, and there exists a
   * confusable identifier in another script. When testing whether two
   * identfiers are confusable, report that they are if each is of a single
   * script, the scripts of the two identifiers are different, and the
   * identifiers are visually confusable.
   */
  public static final int WHOLE_SCRIPT_CONFUSABLE = 4;

  /**
   * Any Case Modifier for confusable identifier tests. If specified, consider
   * all characters, of any case, when looking for confusables. If ANY_CASE is
   * not specified, identifiers being checked are assumed to have been case
   * folded. Upper case conusable characters will not be checked. Selects
   * between Lower Case Confusable and Any Case Confusable.
   */
  ...
  /**
   * Check an identifier for the presence of invisble characters, such as
   * zero-width spaces, or character sequences that are likely not to display,
   * such as multiple occurences of the same non-spacing mark. This check does
   ...
   * acceptable characters. See uspoof_setAllowedChars() and
  ...
  /**
   * private constructor: a SpoofChecker has to be build from builder
   */
  ...
  public static class Builder {
    ...
    public Builder(SpoofChecker src) {
      ...
      fChecks = src.fChecks;
      fSpoofData = null;
      fAllowedCharsSet = src.fAllowedCharsSet;
      fAllowedLocales = new LinkedHashSet<ULocale>();
     ...
     * inputs correspond to the Unicode data files confusables.txt
     ...
     *            unicode.org.
     * @throws SpoofCheckerException
     *             Among the possible errors: -- PARSE_ERROR, which is used
     *             to report syntax errors in the input. -- which of the
     *             input files contains the error. The value is one of
     ...
     * 
     * @param locales
     *            A Set of locales, from which the language and associated
     *            script are extracted. The locales are syntactically like
     *            those from the HTTP Accept-Language header. If the locales
     *            Set is null, no restrictions will be placed on the allowed
     *            characters.
     * 
     * @return self
     * @draft ICU 4.4
    ...
    // setAllowedLocales.
    // Given a locale (a language), add all the characters from all of the
    // scripts used with that language
    // to the allowedChars UnicodeSet
    void addScriptChars(ULocale locale, UnicodeSet allowedChars) {
     ...
     * 
     * @param chars
     *            A Unicode Set containing the list of charcters that are
   ...
   * perform. The returned set is immutable.
   * 
   ...
   * 
   * setAllowedChars() will reset the list of allowed to be empty.
   * 
   ...
   * the caller.
   * 
   * The returned set will become invalid if the spoof detector is closed, or
   * if a new set of allowed characters is specified.
   * 
  ...
  public static class CheckResult {
    /*
     * status is a set of enum SpoofCheck values for any potential security
     * or spoofing issues detected.
     */
    public int status;
    /*
     * position is the index of the first string position that fails the
     * allowed character limitation checks.
     */
    public int position;
  }
   ...
   * to be performed is specified with setChecks().
   * 
   * @param text
   ...
   * @return If there is any issue found with the input string.
   */
  public boolean check(String text, CheckResult checkResult) {
    ...
    if (0 != (this.fChecks & (WHOLE_SCRIPT_CONFUSABLE
      ...
      NFKDBuffer normalizedInput = new NFKDBuffer(text);
      String nfkdText = normalizedInput.getBuffer();
   ...
   *            The strings are in UTF-16 format.
   * @param s2
   *            The second of the two strings to be compared for
  ...
  public int areConfusable(String s1, String s2) {
    ...
    // If no tests relavant to this function have been specified, return an
    // error.
   ...
   * 
   * @param type
   *            The type of skeleton, corresponding to which of the Unicode
  ...
  public String getSkeleton(int type, String s) {
    ...
    int inputIndex = 0;
    StringBuffer skelStr = new StringBuffer();
    while (inputIndex < normalizedLen) {
      ...
      // buffer to hold the Unicode defined skeleton mappings for a single
      // code point
      char buf[] = this.confusableLookup(c, tableMask);
      skelStr.append(buf);
    }
  ...
  private char[] confusableLookup(int inChar, int tableMask) {
    ...
    if (!foundChar) { // Char not found. It maps to itself.
      return Character.toChars(inChar);
    }
    ...
    destBuf = new char[stringLen];
    System.arraycopy(fSpoofData.fCFUStrings, value, destBuf, 0, stringLen);
    return destBuf;
  }

  /** parse a hex number. Untility used by the builders. */
  // Convert a text format hex number. Utility function used by builder code.
  // Static.
  // Input: String text.
  // Output: a int, the codepoint value
  // Input has been pre-checked, and will have no non-hex chars.
  // The number must fall in the code point range of 0..0x10ffff
  // Static Function.
  static int ScanHex(String s) throws SpoofCheckerException {
    int start = 0;
    int limit = s.length();
    ...
    for (i = start; i < limit; i++) {
      int digitVal = s.charAt(i) - 0x30;
      if (digitVal > 9) {
        digitVal = 0xa + (s.charAt(i) - 0x41); // Upper Case 'A'
      }
      if (digitVal > 15) {
        digitVal = 0xa + (s.charAt(i) - 0x61); // Lower Case 'a'
      }
      assert (digitVal <= 0xf);
      val <<= 4;
      val += digitVal;
    ...
    if (val > 0x10ffff) {
      // status = U_PARSE_ERROR;
      throw new SpoofCheckerException();
    }
    return val;
  }
  ...
  void wholeScriptCheck(CharSequence text, ScriptSet result) {
    ...
    while (inputIdx < text.length()) {
      ...
      if (index == 0) {
        ...
        // TODO: we should change the data to have sets with just the
        // single script
        // bit for the script of this char. Gets rid of this special
        // case.
        // Until then, grab the script from the char and intersect it
        // with the set.
