...
public class TimeZoneFormat extends UFormat implements Freezable<TimeZoneFormat>, Serializable {
     ...
     * </ul>
     * This format always includes 2 digit hours and minutes. When the given offset has non-zero
     * seconds field, 2 digit seconds field will be appended. For example,
     ...
     * <li>GMT zero format (e.g. "GMT" - see {@link #getGMTZeroFormat()})
     * </ul>
     * This format uses the shortest representation of offset. The hour field does not
    ...
    public int parseOffsetLocalizedGMT(String text, ParsePosition pos) {
        return parseOffsetLocalizedGMT(text, false, pos, null);
    }
    ...
    public int parseOffsetShortLocalizedGMT(String text, ParsePosition pos) {
        return parseOffsetLocalizedGMT(text, true, pos, null);
    }
    ...
    public TimeZone parse(Style style, String text, ParsePosition pos, EnumSet<ParseOption> options, Output<TimeType> timeType) {
        ...
        if (fallbackLocalizedGMT || fallbackShortLocalizedGMT) {
            ...
            offset = parseOffsetLocalizedGMT(text, fallbackShortLocalizedGMT, tmpPos, hasDigitOffset);
...
//            evaluated |= (fallbackLocalizedGMT ? Style.LOCALIZED_GMT.flag : Style.LOCALIZED_GMT_SHORT.flag);
        ...
        if ((evaluated & Style.LOCALIZED_GMT.flag) == 0) {
            ...
            Output<Boolean> hasDigitOffset = new Output<Boolean>(false);
            offset = parseOffsetLocalizedGMT(text, false, tmpPos, hasDigitOffset);
            if (tmpPos.getErrorIndex() == -1) {
                if (tmpPos.getIndex() == maxPos || hasDigitOffset.value) {
                    pos.setIndex(tmpPos.getIndex());
                    return getTimeZoneForOffset(offset);
        ...
        if ((evaluated & Style.LOCALIZED_GMT_SHORT.flag) == 0) {
            tmpPos.setIndex(startIdx);
            tmpPos.setErrorIndex(-1);

            Output<Boolean> hasDigitOffset = new Output<Boolean>(false);
            offset = parseOffsetLocalizedGMT(text, true, tmpPos, hasDigitOffset);
            if (tmpPos.getErrorIndex() == -1) {
                if (tmpPos.getIndex() == maxPos || hasDigitOffset.value) {
                    pos.setIndex(tmpPos.getIndex());
                    return getTimeZoneForOffset(offset);
        ...
        if (parseAllStyles) {
            ...
            // Try IDs
            if ((evaluated & Style.ZONE_ID.flag) == 0) {
                tmpPos.setIndex(startIdx);
                tmpPos.setErrorIndex(-1);
            ...
            if ((evaluated & Style.ZONE_ID_SHORT.flag) == 0) {
                tmpPos.setIndex(startIdx);
                tmpPos.setErrorIndex(-1);
    ...
    /**
     * Truncates minute field from the offset pattern with hour/minute
     * 
     * @param offsetHM the offset pattern including hour and minute fields
     * @return the offset pattern including only hour field
     */
     ...
     * offset format string.
     */
    private int parseOffsetLocalizedGMT(String text, boolean isShort, ParsePosition pos, Output<Boolean> hasDigitOffset) {
